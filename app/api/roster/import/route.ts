...
export async function POST(request:Request){
  try{
    const {user,program}=await requireProgramContext(request.headers); const body=await request.json() as Record<string,unknown>; if(typeof body.sourceUrl!=='string')return Response.json({error:'Roster URL is required.'},{status:400});
    ...
    const summary=await upsertRosterEvidence({programId:program.programId,seasonId:program.seasonId,teamId:program.teamId,players,sourceFamily:'official_roster',actorEmail:user.email,sourceArtifactId:source.id}); return Response.json({summary,sourceArtifactId:source.id,duplicateSource:source.duplicate});
